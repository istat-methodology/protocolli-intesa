...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pipeline S4 aggiornata: legge prima i soggetti puliti prodotti dalla V4")
    parser.add_argument("--input-json", required=True, help="Percorso al file JSON in input")
    parser.add_argument("--soggetti-csv", required=True, help="Percorso a soggetti_unici_puliti.csv prodotto dalla V4")
    parser.add_argument("-o", "--output-dir", default="output_reti_violenza_v4", help="Cartella di output")
    parser.add_argument("--figura1-by-macro", action="store_true", help="Genera Figura 1 aggregata per macro_tipologia")
    parser.add_argument("--no-png", action="store_true", help="Non genera i grafici PNG")
    args = parser.parse_args()

    input_path = Path(args.input_json)
    soggetti_csv_path = Path(args.soggetti_csv)
