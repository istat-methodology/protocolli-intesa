...
OUTPUT_JSON = Path(r"output\data\step_3.0\3.0_risultati_enriched_merged_geo.json")
...
PLACEHOLDERS = {"", "nan", "none", "null", "nd", "n.d.", "n/d", "non disponibile", "non indicato"}
PROVINCIA_KEYWORDS = ["provincia", "citta metropolitana", "città metropolitana", "libero consorzio"]
REGIONE_KEYWORDS = ["regione", "provincia autonoma"]
COMUNE_KEYWORDS = ["comune", "municipio"]
...
def load_comuni_index(comuni_file: str | Path):
    ...
    return {"by_name": by_name, "by_istat": by_istat}
...
def load_province_index(province_file: str | Path):
    ...
    return {"by_sigla": by_sigla, "by_name": by_name, "by_codice": by_codice}
...
def role_in_text(ent: dict, label: str) -> bool:
    ...
    txt = txt.lower().replace("_", " ").replace("-", " ").replace("/", " ")
...
def extract_role_flags(ent: dict) -> dict:
    is_firmatario = role_in_text(ent, "firmatario") or is_true_like(ent.get("ruolo_firmatario"))
    is_proponente = (
        role_in_text(ent, "proponente")
        or role_in_text(ent, "soggetto proponente")
        or is_true_like(ent.get("ruolo_proponente"))
    )
    is_attore = role_in_text(ent, "attore") or is_true_like(ent.get("ruolo_attore"))

    is_gestione = (
        note_has(ent, [r"\bgestisc\w*", r"\bgestion\w*", r"\bente gestore\b", r"\bsoggett[oi] gestor\w*"])
        or is_true_like(ent.get("ruolo_gestione"))
        or is_true_like(ent.get("gestione"))
    )
    is_monitoraggio = (
        note_has(ent, [r"\bmonitor\w*", r"\bmonitora\w*", r"\bmonitoraggio\b"])
        or is_true_like(ent.get("ruolo_monitoraggio"))
        or is_true_like(ent.get("monitoraggio"))
    )
    is_coordinamento = (
        note_has(ent, [r"\bcoordina\w*", r"\bcoordinament\w*", r"\bcoordinatore\b", r"\bcoordinatric\w*"])
        or is_true_like(ent.get("ruolo_coordinamento"))
        or is_true_like(ent.get("coordinamento"))
    )

    is_governance_generica = note_has(
        ent,
        [
            r"\bpartecipa\s+alla\s+governance\b",
            r"\bfa\s+parte\s+della\s+governance\b",
            r"\bcomponente\s+della\s+governance\b",
            r"\bmembro\s+della\s+governance\b",
            r"\bgovernance\s+della\s+rete\b",
        ],
    )

    is_ente_capofila = (
        is_true_like(ent.get("ente_capofila"))
        or is_true_like(ent.get("ruolo_capofila"))
        or is_true_like(ent.get("capofila"))
        or note_has(ent, [r"\bente\s+capofila\b", r"\bsoggetto\s+capofila\b", r"\bcapofila\b"])
    )

    is_governance = is_coordinamento or is_gestione or is_monitoraggio or is_governance_generica
    return {
        ...
        "is_governance_generica": is_governance_generica,
        "is_governance": is_governance,
        "is_ente_capofila": is_ente_capofila,
    }


def build_ruolo_list(flags: dict) -> list[str]:
    out = []
    if flags["is_firmatario"]: out.append("firmatario")
    if flags["is_proponente"]: out.append("proponente")
    if flags["is_attore"]: out.append("attore")
    if flags["is_gestione"]: out.append("gestione")
    if flags["is_monitoraggio"]: out.append("monitoraggio")
    if flags["is_coordinamento"]: out.append("coordinamento")
    if flags["is_governance_generica"] and not any(flags[k] for k in ["is_gestione", "is_monitoraggio", "is_coordinamento"]):
        out.append("governance_generica")
    if flags["is_ente_capofila"]: out.append("ente_capofila")
    return out
...
def build_governance_detail(flags: dict) -> str:
    ...
    if flags["is_monitoraggio"]: out.append("monitoraggio")
    if flags["is_coordinamento"]: out.append("coordinamento")
    if flags["is_governance_generica"] and not out:
        out.append("governance generica")
    return ", ".join(out)
...
def detect_level_from_entity(ent: dict) -> str:
    ...
    blob = " | ".join([nome, tipo])
    if any(k in blob for k in REGIONE_KEYWORDS): return "regione"
    if any(k in blob for k in PROVINCIA_KEYWORDS): return "provincia"
    if any(k in blob for k in COMUNE_KEYWORDS): return "comune"
...
def enrich_entity_geo_and_roles(ent: dict, comuni_index: dict, province_index: dict, regioni_index: dict) -> tuple[dict, str]:
    if not isinstance(ent, dict):
        return ent, "no_match"

    flags = extract_role_flags(ent)
    ent["ruolo"] = build_ruolo_list(flags)
    ent["governance_detail"] = build_governance_detail(flags)
    for k, v in flags.items():
        ent[k] = v

    livello = clean_text(ent.get("livello_territoriale")) or detect_level_from_entity(ent)
    ...
    if livello == "comune":
        match = best_match_comune(comuni_index["by_name"].get(comune_norm, []), provincia_raw)
        if not match and nome_norm:
            match = best_match_comune(comuni_index["by_name"].get(nome_norm, []), provincia_raw)
        if match:
            ...
            if not already_has_coords:
                lat = try_float(match.get("lat")); lon = try_float(match.get("lon"))
                if lat is not None and lon is not None:
                    ent["lat"] = lat; ent["lon"] = lon
            return ent, "comune"
    ...
    if livello == "regione":
        ...
        if match:
            ...
            if not clean_text(ent.get("codice_regione")):
                ent["codice_regione"] = clean_text(match.get("codice_regione"))
            if not already_has_coords:
                lat = try_float(match.get("lat_centroide")); lon = try_float(match.get("lon_centroide"))
                if lat is not None and lon is not None:
                    ent["lat"] = lat; ent["lon"] = lon
            return ent, "regione"

    match_comune = best_match_comune(comuni_index["by_name"].get(comune_norm or nome_norm, []), provincia_raw)
    if match_comune:
        if not clean_text(ent.get("comune")):
        ...
        if not already_has_coords:
            lat = try_float(match_comune.get("lat")); lon = try_float(match_comune.get("lon"))
            if lat is not None and lon is not None:
                ent["lat"] = lat; ent["lon"] = lon
        return ent, "comune"

    match_prov = province_index["by_sigla"].get(provincia_norm) if provincia_norm else None
    ...
    match_reg = best_match_named(regioni_index["by_name"].get(regione_norm, [])) if regione_norm else None
    ...
    if match_reg:
        ...
        if not already_has_coords:
            lat = try_float(match_reg.get("lat_centroide")); lon = try_float(match_reg.get("lon_centroide"))
            if lat is not None and lon is not None:
                ent["lat"] = lat; ent["lon"] = lon
        return ent, "regione"
...
def enrich_with_geo_ruoli(input_file: str | Path = INPUT_JSON, output_file: str | Path = OUTPUT_JSON, comuni_file: str | Path = COMUNI_JSON, province_file: str | Path = PROVINCE_JSON, regioni_file: str | Path = REGIONI_JSON) -> str:
    ...
    tot_entities = coords_before = coords_after = 0
    count_comune = count_provincia = count_regione = count_no_match = 0
    ruolo_counts = {k: 0 for k in ["firmatario", "proponente", "attore", "gestione", "monitoraggio", "coordinamento", "governance", "governance_generica", "ente_capofila"]}
    ...
    for item in iter_items(risultati):
        for ent in iter_entities(item):
            ...
            if has_valid_coords(ent):
                coords_before += 1
            ent, esito = enrich_entity_geo_and_roles(ent, comuni_index, province_index, regioni_index)
            if has_valid_coords(ent):
                coords_after += 1
            if ent.get("is_firmatario"): ruolo_counts["firmatario"] += 1
            if ent.get("is_proponente"): ruolo_counts["proponente"] += 1
            if ent.get("is_attore"): ruolo_counts["attore"] += 1
            if ent.get("is_gestione"): ruolo_counts["gestione"] += 1
            if ent.get("is_monitoraggio"): ruolo_counts["monitoraggio"] += 1
            if ent.get("is_coordinamento"): ruolo_counts["coordinamento"] += 1
            if ent.get("is_governance"): ruolo_counts["governance"] += 1
            if ent.get("is_governance_generica"): ruolo_counts["governance_generica"] += 1
            if ent.get("is_ente_capofila"): ruolo_counts["ente_capofila"] += 1
            if esito == "comune": count_comune += 1
            elif esito == "provincia": count_provincia += 1
            elif esito == "regione": count_regione += 1
            else: count_no_match += 1
    ...
    print(f"Governance generica: {ruolo_counts['governance_generica']}")
    print(f"Ente capofila: {ruolo_counts['ente_capofila']}")
    return str(output_path)


if __name__ == "__main__":
    enrich_with_geo_ruoli(INPUT_JSON, OUTPUT_JSON, COMUNI_JSON, PROVINCE_JSON, REGIONI_JSON)
